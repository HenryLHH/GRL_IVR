...
class Trainer(nn.Module):
    def __init__(self, hyperparameters):
        ...
        ''' 
        a for roads with cars, while b for empty roads, content is road, style is car
        '''
    ...
    def gen_update(self, x_a, x_b, hyperparameters):
        ...
        # decode (cross domain)
        # TODO gen_a.decode h_b or h_b + n_b ???
        if hyperparameters["gen_a_Decode_With_n_b"] == 1:
            x_ba = self.gen_a.decode(h_b + n_b, s_a)
            x_ab = self.gen_b.decode(c_a)
        else:
            x_ba = self.gen_a.decode(h_b, s_a)
            x_ab = self.gen_b.decode(c_a + n_b)
        # encode again
        c_b_recon, s_a_recon = self.gen_a.encode(x_ba)
        c_a_recon, n_recon = self.gen_b.encode(x_ab)
        # decode again (if needed)
        # TODO gen_a.decode h_b or h_b + n_b ???
        if hyperparameters["gen_a_Decode_With_n_b"] == 1:
            x_aba = self.gen_a.decode(c_a_recon + n_recon, s_a_prime) if hyperparameters['recon_x_cyc_w'] > 0 else None
            x_bab = self.gen_b.decode(c_b_recon) if hyperparameters['recon_x_cyc_w'] > 0 else None
        else:
            x_aba = self.gen_a.decode(c_a_recon, s_a_prime) if hyperparameters['recon_x_cyc_w'] > 0 else None
            x_bab = self.gen_b.decode(c_b_recon + n_recon) if hyperparameters['recon_x_cyc_w'] > 0 else None
        ...
        self.loss_gen_recon_kl_cyc_bab = self.__compute_kl(c_b_recon)
    ...
    def sample(self, x_a, x_b):
        ...
        for i in range(x_a.size(0)):
            ...
            x_ba1.append(self.gen_a.decode(h_b, s_a1[i].unsqueeze(0)))
            x_ba2.append(self.gen_a.decode(h_b, s_a2[i].unsqueeze(0)))
            x_ab1.append(self.gen_b.decode(c_a))
            x_ab2.append(self.gen_b.decode(c_a + Variable(torch.randn(c_a .size()).cuda(c_a .data.get_device()))))
