...
def generate_random_lines(imshape,slant,drop_length):    
    ...
    for i in range(50): ## If You want heavy rain, try increasing this        
        ...
        y= np.random.randint(10,imshape[0]-10-drop_length)
        drops.append((x,y))
    return drops
    
def add_circles(image, slant):
    imshape = image.shape
    
    drop_length=2  
    drop_width=2
    drop_color=(200,200,20) ## a shade of gray    
...
class WarpFrame(gym.ObservationWrapper):
    ...
    def __init__(self, env, change_style=False, munit_model_fn=None):
        super().__init__(env)
        self.size = 84
        ...
        if self.change_style:
            self.slant = 0 # np.random.randint(-5,5)
    ...
    def observation(self, frame):
        ...
        if self.change_style:
            frame = add_circles(frame.copy(), self.slant)
...
def wrap_deepmind(
    ...
    if warp_frame:
        env = WarpFrame(env, change_style, munit_model_fn)
    if scale:
...
def make_atari_env(task, seed, training_num, test_num, mixed=False, munit_model_fn=None, **kwargs):
    """Wrapper function for Atari env.
    If EnvPool is installed, it will automatically switch to EnvPool's Atari env.
    :return: a tuple of (single env, training envs, test envs).
    """
    if mixed:
        env = wrap_deepmind(task, change_style=False)
        train_envs = ShmemVectorEnv(
            [
                lambda:
                wrap_deepmind(task, episode_life=True, clip_rewards=True, 
                    change_style=False)
        ...
        test_envs = ShmemVectorEnv(
            [
                lambda:
                wrap_deepmind(task, episode_life=False, clip_rewards=False, change_style=False)
                for _ in range(test_num//2)
            ] +
            [
                lambda:
                wrap_deepmind(task, episode_life=False, clip_rewards=False, 
                    change_style=True, munit_model_fn=munit_model_fn)
                for _ in range(test_num//2)
            ]
        )
        ...
        train_envs = ShmemVectorEnv(
            [
                lambda:
                wrap_deepmind(task, episode_life=True, clip_rewards=True, change_style=False)
